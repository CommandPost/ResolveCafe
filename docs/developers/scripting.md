...
This page is dedicated to Lua & Python scripting in DaVinci Resolve.

Whilst a lot of this page is currently just a "copy and paste" job from the existing documentation, the intention is to expand upon and enhance it - filling in all the gaps.
...
**Affects:** `x = Project:GetSetting('superScale')` and `Project:SetSetting('superScale', x)`
...
**Affects:** `x = MediaPoolItem:GetClipProperty('Super Scale')` and `MediaPoolItem:SetClipProperty('Super Scale', x)`
